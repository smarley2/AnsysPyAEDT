...
| Guided Studio UI (M7c) | **Implementation complete, awaiting Fabio Posser's verification.** The step rail runs `Core & Material` → `Windings` → `Preliminary` → `Simulation` → `Review`, with bidirectional core/material filtering, native numeric validators, live read-only preliminary estimates in engineering units, and the `Show solver window` / `Open generated file` / `Open run folder` actions. `Open Material Studio` opens the material library in its own separate window, not a step, and refreshes the selector's choices when it closes. |
