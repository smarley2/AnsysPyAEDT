...
- **D10 — Export integration:** `MaterialSpec` gains `bh_curve: tuple[tuple[float, float], ...] = ()` (B, H pairs — pyaedt's order) and `steinmetz: SteinmetzFitSpec | None = None`. A project pins one explicit `MaterialRevisionSelection` for the core's `MaterialRef`; exactly one matching entry is resolved and multiple matching revisions raise typed `MaxwellExportBlocked`. Export never silently chooses a latest revision. M5b will list all revisions and may suggest the latest APPROVED revision, but must persist the user's explicit selection in the project. When no matching entry exists, the M3 grade fallback still applies (with a note). `material_spec_from_material_record` accepts exactly one B-H series; multiple `BH_CURVE` series are ambiguous and blocked until M5b adds condition selection. **Approved B-H records unblock ferrite cores.** Adapters — verified against installed libs: pyaedt nonlinear B-H = `material.permeability = [[b, h], ...]` (documented list-of-pairs setter); Steinmetz = `material.set_power_ferrite_coreloss(cm=k, x=alpha, y=beta)`, whose return must be truthy or the material stage fails; pyFEMM B-H = singular `mi_addbhpoint(name, b, h)` once per point, never a plural bulk call. DC-bias linear-material caveat note drops when a B-H record is applied.
...
`simulation/femm_problem.py` `FemmMaterial` gains `bh_points: tuple[tuple[float, float], ...] = ()` ((B,H) pairs), populated from the plan's core MaterialSpec; `adapters/femm/solver.py` after `mi_addmaterial` calls singular `femm.mi_addbhpoint(material.name, b, h)` once for every `(b, h)` point, never a plural bulk API (μ args in `mi_addmaterial` stay as the scalar for the linear fallback FEMM needs pre-BH). Fakes are faithful: PyAEDT core-loss setters return `True` by default and can return `False` for material-stage failure tests; the fake FEMM module exposes singular `mi_addbhpoint`.

Tests: spec-from-record approved happy + non-approved refused + ferrite CoreRecord with approved record passes + multiple B-H series refused; plan builders thread the record; export services refuse multiple pinned revisions for the selected core ref but preserve zero-match grade fallback; adapter unit tests assert the nonlinear assignment shape, `set_power_ferrite_coreloss` args and truthy return requirement (3D + 2D), and one singular FEMM `mi_addbhpoint` call per B-H point; manifest block extended assertions; existing grade-fallback tests stay green (default params).
...
2. In M5b, list all material revisions and suggest the latest APPROVED one, then explicitly pin the chosen revision in the project's single matching schema v3 materials entry. Generate Maxwell 3D + FEMM and open both — check the nonlinear B-H curve and power-ferrite core-loss coefficients landed in AEDT (Material properties dialog), and FEMM shows every point added through singular `mi_addbhpoint(name, b, h)`. Export must block duplicate pinned entries and never silently choose latest.
...
- Verified-API facts baked in: pyaedt nonlinear `material.permeability = [[b, h], ...]` and `set_power_ferrite_coreloss(cm, x, y)` (read from installed pyaedt 1.2 source), with a required truthy setter result; pyFEMM exposes singular `mi_addbhpoint(name, b, h)` and requires one call per point, never a plural call.
- Export resolution is deterministic and explicit: zero matching project selections preserve grade fallback, one uses the pinned revision, and multiple matching selections raise typed `MaxwellExportBlocked`. M5b may suggest latest APPROVED but must save the explicit revision; export never resolves latest.
- Exactly one B-H series can export. Multiple `BH_CURVE` series are blocked as ambiguous until M5b provides condition selection.
